...
public class GaussJordanElimination {
    ...
    private static int N=0;        //N-by-N system

    public GaussJordanElimination(int N) {
        this.N = N;
    }
    private static double[][] a;       //N-by-N+1 augmented matrix
    ...
    public GaussJordanElimination(double [][] A, double [] b){
        ...
        //only if you need to find certificate of infeasibility
        for(int i = 0; i < N; i++)
            a[i][N + i] = 1.0;
    ...
    //scale row p(ok to go from q+1 to N but do this for consistency with)
    for(int j = 0; j <= N+N; j++)
        if(j != q)
            a[p][j] /= a[p][q];
    a[p][q] = 1.0;
...
//                System.out.print(" "+(int)a[i][j]);
//            }
            System.out.println((int)a[i][N+N]);
            xy+=Integer.toString((int)a[i][N+N])+";";
        }
        xy=xy.substring(0,xy.lastIndexOf(";"));
        xy+="]";
        return xy;
    }
    
    //chech that Ax = b or yA = 0, yb! = 0
...
////        }
//          
//        //System.out.println(Matrix.display(Console.evaluate(test(mat, constants[0]))));
//        GaussJordanElimination gaussJordanElimination = new GaussJordanElimination(mat, constants[0]);
//        //System.out.println(Matrix.display(Console.evaluate(gaussJordanElimination.show())));
